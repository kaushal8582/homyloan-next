...
const RentVsBuyCalculator = () => {
  ...
  const [startDate, setStartDate] = useState("May 2024");
  ...
  useEffect(() => {
    try {
      const savedState = localStorage.getItem("rentVsBuyCalculatorState");
      ...
      if (savedState) {
        ...
        if (parsed.monthlyDebts !== undefined)
          setMonthlyDebts(parsed.monthlyDebts);
        if (parsed.downPaymentMode !== undefined)
          setDownPaymentMode(parsed.downPaymentMode);
        if (parsed.downPaymentPercent !== undefined)
          setDownPaymentPercent(parsed.downPaymentPercent);
        if (parsed.monthlyDebtsMode !== undefined)
          setMonthlyDebtsMode(parsed.monthlyDebtsMode);
        if (parsed.monthlyDebtsPercent !== undefined)
          setMonthlyDebtsPercent(parsed.monthlyDebtsPercent);
        if (parsed.propertyTaxMode !== undefined)
          setPropertyTaxMode(parsed.propertyTaxMode);
        if (parsed.propertyTaxPercent !== undefined)
          setPropertyTaxPercent(parsed.propertyTaxPercent);
        if (parsed.homeInsuranceMode !== undefined)
          setHomeInsuranceMode(parsed.homeInsuranceMode);
        if (parsed.homeInsurancePercent !== undefined)
          setHomeInsurancePercent(parsed.homeInsurancePercent);
        ...
        if (parsed.pmi !== undefined) setPmi(parsed.pmi);
        if (parsed.extraPayment !== undefined)
          setExtraPayment(parsed.extraPayment);
        if (parsed.startDate !== undefined) setStartDate(parsed.startDate);
        if (parsed.showExtraPayments !== undefined)
          setShowExtraPayments(parsed.showExtraPayments);
      }
    } catch (error) {
