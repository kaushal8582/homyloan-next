...
const StepThreeForm = ({ formData = {}, onNext, onDataChange }) => {
  const [selected, setSelected] = useState(formData.foundHome ?? null);

  React.useEffect(() => {
    if (onDataChange) onDataChange({ ...formData, foundHome: selected });
  }, [selected]);
        ...
        {options.map((option) => (
          <div
            ...
            onClick={() => {
              ...
              onDataChange?.({ ...formData, foundHome: option.id });
              onNext?.();
            }}
            className={`
