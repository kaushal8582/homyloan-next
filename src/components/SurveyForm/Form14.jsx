...
const SpouseForm = ({ formData = {}, onNext, onDataChange }) => {
  const [selected, setSelected] = useState(formData.spouseOnLoan ?? null);

  React.useEffect(() => {
    if (onDataChange) onDataChange({ ...formData, spouseOnLoan: selected });
  }, [selected]);
  ...
  return (
    <div className="flex flex-col items-start p-4 sm:p-10  max-w-4xl mx-auto ">
      ...
      <div className="w-full max-w-[664px] mb-12">
        ...
        Will your spouse be on the loan?
        </h1>
      </div>
        ...
        {options.map((option) => (
          <div
            ...
            onClick={() => {
              ...
              onDataChange?.({ ...formData, spouseOnLoan: option.id });
              onNext?.();
            }}
            className={`
