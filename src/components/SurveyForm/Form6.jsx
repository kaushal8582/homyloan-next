...
const StepSixForm = ({ formData = {}, onNext, onDataChange }) => {
  const [selected, setSelected] = useState(formData.workingWithAgent ?? null);

  React.useEffect(() => {
    if (onDataChange) onDataChange({ ...formData, workingWithAgent: selected });
  }, [selected]);
        ...
        {options.map((option) => (
          <div
            ...
            onClick={() => {
              ...
              onDataChange?.({ ...formData, workingWithAgent: option.id });
              onNext?.(option.id);
            }}
            className={`
