...
const defaultMortgage = {
  ...
  cards: [
    { title: "FHA Loans", iconSrc: "/RefinanceLogo1.svg", description: "Find the right mortgage options to purchase your new home.", route: "/fha-loan", style: { borderTopRightRadius: "30px", borderBottomLeftRadius: "30px", borderTopLeftRadius: "60px", borderBottomRightRadius: "60px" } },
...
const Mortgage_Refinance = ({ content }) => {
  const router = useRouter();
  const mortgage = mergeWithDefaults(defaultMortgage, content?.mortgage);
  const cards =
    mortgage.cards && mortgage.cards.length > 0
      ? mortgage.cards.map((c) => ({ ...c, style: c.style || {} }))
      : defaultMortgage.cards;
  ...
  return (
    <section className="w-full bg-black text-white py-12 sm:py-16 lg:py-20 px-4 sm:px-6 overflow-hidden">
      <div className="max-w-5xl mx-auto text-center px-2">
        ...
        <p className="mt-3 sm:mt-4 text-xs sm:text-sm md:text-base text-white/70 max-w-3xl mx-auto">
          {mortgage.subtitle}
        </p>
      </div>
      ...
      <div className="mt-12 sm:mt-16 lg:mt-20 flex justify-center">
        <Button onClick={() => router.push("/refinance2")} label="Learn More" bgColor="#FF6B6B" dotColor="#1a1a1a" />
      </div>
    </section>
  );
}

export default Mortgage_Refinance
